        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 relative z-10">
            <div class="glass-card p-6 md:p-8 text-white animate-fadeInUp">
                <form method="POST" action="{{ route('announcements.update', $announcement) }}" enctype="multipart/form-data">
                    ...
                    <!-- Gambar Header -->
                    <div class="mb-6">
                        <label for="image" class="block text-sm font-medium text-green-300 mb-2">Ganti Gambar Header (Opsional)</label>
                        @if($announcement->image_path)
                            <p class="text-xs text-gray-400 mb-2">Gambar saat ini:</p>
                            <img src="{{ Storage::url($announcement->image_path) }}" alt="Current Image" class="h-32 mb-2 rounded">
                        @endif
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" id="content" class="w-full ...">{{ old('content', $announcement->content) }}</textarea>
                        @error('content') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('announcements.index') }}" class="px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-md transition">Batal</a>
