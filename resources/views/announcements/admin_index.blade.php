<x-app-layout>
    ...
    <div class="gradient-bg py-8 md:py-12 relative min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 relative z-10">
            <div class="glass-card overflow-hidden animate-fadeInUp z-content">
                <div class="p-4 md:p-6 text-white">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
                        <h3 class="text-lg font-medium">Daftar Berita</h3>
                        <a href="{{ route('announcements.create') }}" class="btn-primary px-4 py-2 md:px-6 md:py-3 text-sm md:text-base w-full md:w-auto text-center">
                            + Tambah Berita Baru
                        </a>
                    </div>
                    @if($announcements->isEmpty())
                        <p class="text-gray-300 text-center py-8">Belum ada berita.</p>
                    @else
                        <div class="overflow-x-auto">
                            <table class="w-full min-w-[600px]">
                                <thead class="table-header">
                                    <tr>
                                        <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-green-300 uppercase tracking-wider">Judul</th>
                                        <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-green-300 uppercase tracking-wider">Dibuat Oleh</th>
                                        <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-green-300 uppercase tracking-wider">Tanggal</th>
                                ...
                                <tbody class="divide-y divide-gray-700">
                                    ...
                                    <tr class="table-row">
                                        ...
                                        <td class="px-3 md:px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            <a href="{{ route('announcements.edit', $announcement) }}" class="text-indigo-400 hover:text-indigo-300">Edit</a>
                                            <form action="{{ route('announcements.destroy', $announcement) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="text-red-400 hover:text-red-300" onclick="return confirm('Yakin ingin menghapus berita ini?')">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
